  ...
  addTeamToTournament(): void {
    ...
    this.tournamentService.addTeamToTournament(tournamentId, teamId).subscribe(
      ...
      error => {
        ...
        } else if (error.status === 400 && error.error.includes('already part of the tournament')) {
          alert('Team is already part of the tournament.');
        } else if (error.status === 400 && error.error.includes('must have at least 5 players')) {
